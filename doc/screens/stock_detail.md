...
| 4 | 2026-04-05 | 全体 | IFRS等複数会計基準対応（会計基準バッジ、ラベル動的変更）を追加 | antigravity |
| 5 | 2026-04-23 | 全体 | 投資戦略・ニュースメモ機能（画像添付・拡大）を追加 | antigravity |
...
| 2 | 企業名ラベル | - | データ表示 | - | - | - | \&nbsp; |
| 3 | 証券コードラベル | - | データ表示 | - | - | - | 4桁 |
| 3.1 | 会計基準バッジ | - | データ表示 | - | - | - | IFRS/J-GAAP等の表示 |
| 4 | 現在株価 | - | データ表示 | - | - | - | リアルタイム株価 |
| 5 | PER | - | データ表示 | - | - | - | 株価 / EPS |
| 6 | PBR | - | データ表示 | - | - | - | 株価 / BPS |
| 7 | 売上高・利益グラフ | - | 画像表示 | - | - | - | BarChart表示（IFRS時、経常利益ラベルを動的変更） |
...
| 18 | 投資戦略・ニュースメモ入力 | ○ | テキストエリア | - | - | - | 自由記述メモ |
| 19 | 画像添付ボタン | ○ | ファイル選択 | - | - | - | プレビュー表示あり |
| 20 | メモ追加ボタン | ○ | ボタン | - | - | - | メモを保存 |
| 21 | メモタイムライン | - | データ表示 | - | - | - | 投稿日時、本文、画像 |
| 22 | メモ削除ボタン | ○ | ボタン | - | - | - | ゴミ箱アイコン（ホバー時表示） |
| 23 | 画像拡大表示（Lightbox） | ○ | モーダル表示 | - | - | - | 画像クリック時に全画面表示 |
...
3. ユーザーのメモ一覧を取得する
   - バックエンド「銘柄別メモ取得API」をコールする
   →画面表示：企業名、株価、各種指標、成長率グラフ、CAGR、メモ履歴を反映（処理終了）
...
| :--- | :--- |
| メモ本文 | 項目No.18の値 |
| 画像ファイル | 項目No.19で選択されたファイル |

</div>

**処理**
1. メモを投稿する
   - バックエンド「銘柄別メモ投稿API」をコールする
   - `multipart/form-data` 形式で送信
2. メモ一覧を再取得またはローカル状態に反映
   →画面表示：入力欄をクリアし、タイムラインの先頭に追加（処理終了）

### No.22 メモ削除ボタン押下

**処理**
1. 削除確認ダイアログを表示
2. 承認された場合、バックエンド「銘柄別メモ削除API」をコール
   →画面表示：対象のメモをタイムラインから削除（処理終了）

### No.23 画像クリック

**処理**
1. 画像拡大モーダル（Lightbox）を表示
   →画面表示：フルスクリーンで画像を表示（処理終了）
